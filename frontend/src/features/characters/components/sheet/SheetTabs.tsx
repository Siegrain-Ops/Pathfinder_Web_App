...
const COLOR_ACTIVE: Record<TabColor, string> = {
  amber:   'border-amber-500   text-amber-200   bg-amber-950/50  font-semibold',
  red:     'border-red-500     text-red-200     bg-red-950/50    font-semibold',
  emerald: 'border-emerald-500 text-emerald-200 bg-emerald-950/50 font-semibold',
  violet:  'border-violet-500  text-violet-200  bg-violet-950/50 font-semibold',
}
    ...
    <div className="relative border-b border-stone-800 bg-stone-950">
      <div className="flex overflow-x-auto scrollbar-hide px-1">
        {visibleTabs.map((tab, idx) => {
          ...
          // Separator before first non-colored tab (after the hero group)
          const prevColored   = idx > 0 && visibleTabs[idx - 1].color && !colored
          // Separator before the final colored tabs (dice/levelup)
          ...
          return (
            <span key={tab.id} className="flex items-center">
              ...
              <button
                ...
                className={clsx(
                  // py-4 on mobile gives ≥ 48px touch target; py-3 on desktop
                  'shrink-0 px-4 py-4 sm:py-3 text-xs tracking-wide transition-all duration-150 border-b-2 -mb-px',
                  colored
                    ? isActive ? COLOR_ACTIVE[colored] : COLOR_INACTIVE[colored]
                    : isActive
                      ? 'border-stone-400 text-stone-200 bg-stone-700/50 font-semibold'
                      : 'border-transparent font-medium text-stone-500 hover:text-stone-300 hover:border-stone-600/50 hover:bg-stone-800/20',
                )}
              >
                {tab.label}
              </button>
